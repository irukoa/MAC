...
Reduction utility. <br /> Usage: <br /> `c = b%reduce(op [, dim])`. | Implements a user defined reduction operation along one or all dimensions of the container. | `class(container), intent(in) :: b`: initialized array. <br /> `procedure(intfc), intent(in) :: op`: user defined array operation. <br /> `integer, intent(in) :: dim` (optional): specifies which dimension is to be reduced. `type(container)/type(b%cont_type()), intent(out)`: if `dim` is present, is a `type(container)` object with `c%rank() = b%rank() - 1` and `c%shape()` simmilar to `b%shape()` with dimension `dim` dropped and otherwise the same specifiers as `b`. If `dim` is absent, is a `type(b%cont_type())` scalar object.
